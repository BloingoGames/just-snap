...
func generateCard(suit,pip,fancyName,bloingoEffect=Global.BloingoEffect1,special=false): #instantiate a generic card with the suit, pip and name given. This will be the standard deck
	var cardScene = load("res://scenes/card.tscn")
	var cardInstance = cardScene.instantiate()
	cardInstance.setParams(suit,pip,fancyName,bloingoEffect,special)
